...
export function useSDK(containerId: string, logger: SdkLogger) {
  ...
  const callbacks = {
    onMessage(message: any) {
      const msg = message?.message ?? JSON.stringify(message)
    ...
    onStatusChange(status: any) {
      ...
      const map: Record<number, string> = { 0: '在线', 1: '离线', 2: '关闭', 3: '可见', 4: '不可见', 5: '在线' }
      performanceStats.value = {
        ...performanceStats.value,
      ...
      // 从离线恢复在线时，flush 缓冲的 speak 消息
      // status: 0=在线 1=离线 2=关闭 3=可见 4=不可见
      // 只有 1/2 是真正的离线状态，3/4 是可见性变化不影���播报
      if (status === 0 || status === 5) {
        _isOnline = true
        // 不自动 flush——SDK 刚恢复在线时音频通道可能未就绪
        // 留给用户手动点「④ 播放」调 flushSpeakQueue()
      } else if (status === 1 || status === 2) {
        _isOnline = false
      }
      // SDK 回调里的"离线"可能是 speak 结束后的正常状态切换，不应影响队列
    },
  ...
  function executeSsml(ssml: string) {
    ...
    if (!sdk || !isInitialized.value || !_isOnline) {
  ...
  function flushSpeakQueue() {
    ...
    while (_pendingSpeak.length > 0) {
      const ssml = _pendingSpeak.shift()!
      executeSsml(ssml)
    }
  }

  // 等待 SDK 状态回调确认在线（status=0|5），超时默认 10s
  function waitForOnline(timeout = 10000): Promise<boolean> {
    return new Promise((resolve) => {
      if (currentStatus.value === 0 || currentStatus.value === 5) {
        resolve(true)
        return
      }
      const stop = watch(currentStatus, (status) => {
        if (status === 0 || status === 5) {
          stop()
          resolve(true)
        }
  ...
  function online() {
    ...
    addApiCall('onlineMode', '{}')
    addCodeSnippet('sdk.onlineMode(); sdk.changeAvatarVisible(true)')
    addLog('状态', '数字人正在恢复在线...')
    _isOnline = true
    // flush 由 onStatusChange(status=0|5) 回调触发
  }

  function destroy() {
    stopFpsMonitor()
    if (sdkInstance.value) {
      sdkInstance.value.interrupt('user')
      sdkInstance.value.offlineMode()
      sdkInstance.value.destroy()
      addCodeSnippet('sdk.interrupt("user"); sdk.offlineMode(); sdk.destroy()')
