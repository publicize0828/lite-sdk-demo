...
const {
  ...
  initSDK, executeSsml, interrupt, setVolume, offline, destroy, currentSpeakState, currentWalkState,
} = useSDK('#sdk-container', { addLog, addApiCall, addCodeSnippet })

provide(SdkKey, { executeSsml, interrupt, setVolume })
...
// 凭证弹窗（仅魔搭 / 预览模式）
const TRIAL_SECONDS = Number((typeof window !== 'undefined' && window.__ENV__?.VITE_TRIAL_SECONDS) || 300)
const trialSeconds = Math.max(0, TRIAL_SECONDS)
const TRIAL_KEY = '__xmov_trial_start'

// localStorage 持久化试用状态
function getTrialStart(): number {
  try { return Number(localStorage.getItem(TRIAL_KEY)) || 0 } catch { return 0 }
}
function setTrialStart(ts: number) {
  try { localStorage.setItem(TRIAL_KEY, String(ts)) } catch {}
}
function clearTrial() {
  try { localStorage.removeItem(TRIAL_KEY) } catch {}
}

const trialStart = getTrialStart()
const now = Date.now()
const trialExpired = ref(trialStart > 0 && (now - trialStart) >= trialSeconds * 1000)
const showCredModal = ref(isModelScopeMode ? (trialStart === 0 || trialExpired.value) : false)
const credConnecting = ref(false)
const credError = ref('')
let _trialTimer = 0

function startTrialTimer(remainingSec: number) {
  if (trialSeconds <= 0) return
  clearInterval(_trialTimer)
  const ms = remainingSec * 1000
...
// 如果试用中但未过期，自动恢复
if (isModelScopeMode && trialStart > 0 && !trialExpired.value) {
  const elapsed = (now - trialStart) / 1000
  const remaining = Math.max(0, trialSeconds - elapsed)
  if (remaining > 0) {
    startTrialTimer(remaining)
...
async function onCredentialConnect(appId: string, appSecret: string) {
  ...
  if (ok) {
    generateConfig()
    setVolume(volume.value)
    executeSsml('<speak>你好，我是你的数字人助手</speak>')
    showCredModal.value = false
    if (isModelScopeMode && !trialExpired.value) {
      setTrialStart(Date.now())
      startTrialTimer(trialSeconds)
    }
  } else {
